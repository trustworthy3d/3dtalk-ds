(function() {
	...
	cwhApp.controller("PrinterControlsController", ['$scope', '$http', '$location', '$routeParams', 'cwhWebSocket', 'photonicUtils', function ($scope, $http, $location, $routeParams, cwhWebSocket, photonicUtils) {
        ...
        function loadGCodeLog () {
            $http.get("/services/printers/getGCodeLog/" + printerName).success(
                function (data) {
                    controller.gCodeLog = data.message;
                }
            );

            this.gCodeLogSocket = cwhWebSocket.connect("services/gCodeLogNotification", $scope).onJsonContent(
                function (data) {
                    if (data.notificationEvent == "SendMessage") {
                        controller.gCodeLog = data.message;
                    }
                }
            );
            if (gCodeLogSocket == null) {
                $scope.$emit("MachineResponse",  {machineResponse: {command:"Browser Too Old", message:"You will need to use a modern browser to run this application."}});
            }
        }
...
//				controller.gcodeProcessing = response.data.command + ":" + response.data.message + controller.gcodeProcessing;
		...
		// zyd add for parameters -s
        this.overrideLiftFeedSpeed = function overrideLiftFeedSpeed() {
			$http.get("services/printJobs/overrideZLiftFeedSpeed/" + controller.currentPrintJob.id + "/" + controller.currentPrintJob.zliftFeedSpeed).then(gCodeSuccess, errorFunction)
		}
        this.overrideLiftRetractSpeed = function overrideLiftRetractSpeed() {
			$http.get("services/printJobs/overrideZLiftRetractSpeed/" + controller.currentPrintJob.id + "/" + controller.currentPrintJob.zliftRetractSpeed).then(gCodeSuccess, errorFunction)
		}
		// zyd add for parameters -e
